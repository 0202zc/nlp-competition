...
class Transformer1(nn.Module):
    """
    Multi-head 版本的 Transformer layer
    """
    ...
    def __init__(self, max_len=512, hidden_size=768):
        ...
        self.gelu = nn.GELU()
        self.attention = self.MultiHeadAttention(head_nums=self.head_nums, max_len=max_len, hidden_size=hidden_size)
        self.self_out = self.BertSelfOutput(hidden_size)
        self.out = self.BertOutput(hidden_size)
        self.intermediate = self.BertIntermediate(hidden_size)
        self.Tanh = nn.Tanh()

    def forward(self, x):
        attention = self.attention(x)
        attention_out = self.self_out(attention, x)
        attention_out = self.intermediate(attention_out)
...
class Transformer2(nn.Module):
    """
    改进1：用门控机制替换传统FFN；
    改进2：使用Relu^2的弱注意力机制，减轻对attention的依赖。
    """
    ...
    class MultiHeadAttention(nn.Module):
        ...
        def forward(self, x):
            ...
            V = self.v(x).reshape(-1, self.max_len, self.head_nums, self.hidden_size // self.head_nums).permute(0, 2, 1,
                                                                                                                3)
            a = (self.relu(torch.matmul(Q, K.transpose(-2, -1))) ** 2) / (
                    self.max_len * (self.hidden_size // self.head_nums))
            attention = torch.matmul(a, V).transpose(-2, -1)
            attention = attention.transpose(1, 2)
            attention = attention.reshape(-1, self.max_len, self.hidden_size)
            return attention
...
class Transformer3(nn.Module):
    """
    改进1：用门控机制替换传统FFN；
    改进2：将做attention时的hidden_size与head_num解绑定
    """
    ...
    class MultiHeadAttention(nn.Module):
        ...
        def forward(self, x):
            Q = self.q(x).reshape(-1, self.max_len, self.head_nums, self.hidden_size // self.head_nums * 4).permute(0,
                                                                                                                    2,
                                                                                                                    1,
                                                                                                                    3)
            K = self.k(x).reshape(-1, self.max_len, self.head_nums, self.hidden_size // self.head_nums * 4).permute(0,
                                                                                                                    2,
                                                                                                                    1,
                                                                                                                    3)
            V = self.v(x).reshape(-1, self.max_len, self.head_nums, self.hidden_size // self.head_nums).permute(0, 2, 1,
