       ...
       # emb_name这个参数要换成你模型中embedding的参数名
        for name, param in self.model.named_parameters():
            if (param.stop_gradient) and emb_name in name:
                ...
                norms = paddle.norm(param.grad)
                if norms != 0:
                    r_at = epsilon * param.grad / (norms+1e-8)
                    param.add_(r_at)
...
def do_train():
    ...
    num_training_steps = len(ds) / args.train_batch_size * args.epochs
    ...
    for k in range(1, K + 1):
        best_accuracy = 0.0
        save_dir_k = '/home/aistudio/model/kfold'
        train_ds = ppnlp.datasets.dataset.MapDataset(ds[:(int((k - 1)/K * len(ds)))] + ds[(int(k / K * len(ds))):])
        dev_ds = ppnlp.datasets.dataset.MapDataset(ds[(int((k - 1) / K * len(ds))):(int(k / K * len(ds)))])
        train_data_loader = create_dataloader(
            train_ds,
            mode='train',
            batch_size=args.train_batch_size,
        ...
        for epoch in range(1, args.epochs + 1):
            for step, batch in enumerate(train_data_loader, start=1):
                ...
                # 对抗训练
                fgm.attack()
                logits_adv, kl_loss_adv = model(input_ids=input_ids, token_type_ids=token_type_ids)
                ...
                if global_step % args.eval_step == 0 and rank == 0:
                    ...
                    if accuracy > best_accuracy:
                        ...
                        save_dir = os.path.join(save_dir_k, "model_%d_fold" % (k))
                ...
                if global_step == args.max_steps:
                    return
                '''
        # save_dir_k = os.path.join(save_dir_k, "model_%d_fold" % (k))
        # accuracy = evaluate(model, criterion, metric, dev_data_loader)
        accuracy = best_accuracy
        print(accuracy)
        # 错误率
        e1 = 1 - accuracy
        w1 = math.log((1 - e1) / (e1 + 1e-8)) / 2
        print("第 %d 分类器的权重为 %f" % (k, w1))
        # weight_list.append(w1)
        # if not os.path.exists(save_dir_k):
        #     os.makedirs(save_dir_k)
        # save_param_path_k = os.path.join(save_dir_k, 'model_state.pdparams')
        # paddle.save(model.state_dict(), save_param_path_k)
        # tokenizer.save_pretrained(save_dir_k)
        with open(file='/home/aistudio/model/kfold/list.txt', mode='a', encoding='utf8', newline='\n') as fl:
            fl.write(str(w1) + '\n')
