...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels, freeze_embedding=False):
        super(BertForSequenceClassification, self).__init__(config)
        self.config = config
        self.num_labels = num_labels
        self.nezha = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.lstm = nn.LSTM(config.hidden_size, config.hidden_size, num_layers=1, bidirectional=True)
        ...
        self.linear = nn.Linear(config.hidden_size * 4, config.hidden_size)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        if self.output_all_encoded_layers:
            all_hidden_states, pooled_output = self.nezha(input_ids, token_type_ids, attention_mask,
                                                          output_all_encoded_layers=True)

            concatenate = torch.cat(
                (all_hidden_states[-1], all_hidden_states[-2], all_hidden_states[-3], all_hidden_states[-4]), -1)
            concatenate = self.linear(concatenate)
            output = self.transformer(concatenate)
        else:
            # encoder_out, pooled_output = self.nezha(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
            encoder_out, pooled_output = self.nezha(input_ids, token_type_ids, attention_mask,
                                                    output_all_encoded_layers=False)
        ...
        # gru_out = torch.mean(gru_out, 1)

        resnet = torch.cat((pooled_output, output), axis=1)
        # resnet = torch.cat((pooled_output, lstm_out), axis=1)
        # resnet = torch.cat((pooled_output, output), axis=1)
        ...
        if labels is not None:
            ...
            loss_fct = focal_loss.FocalLoss(class_num=self.num_labels)
