...
from modeling_nezha_small_sample import BertForSequenceClassification, WEIGHTS_NAME, CONFIG_NAME
from optimization import BertAdam, warmup_linear
from sklearn.metrics import precision_score, recall_score, f1_score
from sklearn.model_selection._split import StratifiedKFold
from transformers import BertConfig, BertModel
from modeling import BertPrefixForSequenceClassification, BertPromptForSequenceClassification
...
# wandb.init(project='small-sample-data-classification-task', entity="betalemon")
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, position_ids, label_id):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_position_ids, all_label_ids)
        if args.local_rank == -1:
        ...
        test_position_ids = torch.tensor([f.position_ids for f in test_features], dtype=torch.long)
        ...
        for fold, (train_index, valid_index) in enumerate(skf.split(all_label_ids, all_label_ids)):
            ...
            train_position_ids = torch.as_tensor(all_position_ids[train_index], dtype=torch.long)
            ...
            valid_position_ids = torch.as_tensor(all_position_ids[valid_index], dtype=torch.long)
            valid_label = torch.as_tensor(all_label_ids[valid_index], dtype=torch.long)

            train = TensorDataset(train_input_ids, train_input_mask, train_segment_ids, train_position_ids, train_label)
            valid = TensorDataset(valid_input_ids, valid_input_mask, valid_segment_ids, valid_position_ids, valid_label)
            test = TensorDataset(test_input_ids, test_input_mask, test_segment_ids, test_position_ids)
            ...
            if args.trained_model_dir:
                ...
                model = BertPrefixForSequenceClassification(bert_config, num_class=num_labels)
                utils.torch_show_all_params(model)
                utils.torch_init_model(model, os.path.join(args.bert_model, 'pytorch_model.bin'), prefix='bert')
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                    ...
                    input_ids, input_mask, segment_ids, position_ids, label_ids = batch

                    # 正常训练
                    loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, position_ids=position_ids,
                                 labels=label_ids)
                    ...
                    loss_sum = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, position_ids=position_ids,
                                     labels=label_ids)  # 累加对抗训练的梯度
                    if n_gpu > 1:
                        loss_sum = loss_sum.mean()  # mean() to average on multi-gpu.
                    if args.gradient_accumulation_steps > 1:
                ...
                for input_ids, input_mask, segment_ids, position_ids, label_ids in tqdm(valid_loader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        ...
                        tmp_eval_loss = model(input_ids=input_ids, token_type_ids=segment_ids,
                                              attention_mask=input_mask, position_ids=position_ids, labels=label_ids)
                        logits, prob = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, position_ids=position_ids)
                ...
                logger.info(result)
                # if not os.path.exists(os.path.join(args.output_dir, str(fold))):
                #     os.mkdir(os.path.join(args.output_dir, str(fold)))
            ...
            id = 1
            results = []
            for input_ids, input_mask, segment_ids, position_ids in tqdm(test_loader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                position_ids = position_ids.to(device)
