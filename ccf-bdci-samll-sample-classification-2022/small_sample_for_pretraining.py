...
from modeling import NezhaForPretraining, NezhaPrefixForPretraining
from modeling_nezha_small_sample import BertConfig, WEIGHTS_NAME, CONFIG_NAME
from optimization import BertAdam, warmup_linear
from sklearn.metrics import precision_score, recall_score, f1_score

from transformers import NezhaConfig

logging.basicConfig(filename='./log/small-sample-pretraining-%s.log' % (strftime('%Y-%m-%d-%H-%M-%S', localtime(time()))),
                    encoding='utf8',
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, masked_input_ids):
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, my_tokenization=False):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        mask_random = random.random()

        if mask_random <= 0.5:
            r1 = random.random()
            if r1 >= 0.2:
                masked_input_ids[random.randint(0, len(input_ids) - 1)] = 103
            elif 0 <= r1 < 0.1:
                masked_input_ids[random.randint(0, len(input_ids) - 1)] = random.randint(0, 21127)
            elif 0.1 <= r1 < 0.2:
                masked_input_ids = masked_input_ids
        else:
            input_ids = [-100] * len(input_ids)
...
def main():
    ...
    if args.trained_model_dir:
        ...
        print('init model...')
        bert_config = NezhaConfig.from_json_file(os.path.join(args.bert_model, 'config.json'))
        model = NezhaPrefixForPretraining(bert_config)
    ...
    if args.do_train:
        ...
        all_masked_input_ids = torch.tensor([f.masked_input_ids for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_masked_input_ids)
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, masked_input_ids = batch
                ...
                #     # 重载模型字典
                #     state_dict = {}
                #     origin_state_dict = torch.load(os.path.join(args.output_dir, 'pytorch_model.bin'))
                #     for key in origin_state_dict:
                #         val = origin_state_dict[key]
                #         new_key = key.replace('.bert', "")
                #         state_dict[new_key] = val
            ...
            # torch.save(model_to_save, "./trained_models/pytorch_model.bin")
