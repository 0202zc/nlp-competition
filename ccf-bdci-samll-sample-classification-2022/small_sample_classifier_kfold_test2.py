...
# from modeling_nezha_small_sample import BertForSequenceClassification, BertConfig, WEIGHTS_NAME, CONFIG_NAME
from optimization import BertAdam, warmup_linear
from sklearn.metrics import precision_score, recall_score, f1_score
from sklearn.model_selection._split import StratifiedKFold
from modeling import XLNetForSequenceClassification, XLNetConfig, WEIGHTS_NAME, CONFIG_NAME
    ...
    if args.do_train:
        ...
        for fold, (train_index, valid_index) in enumerate(skf.split(all_label_ids, all_label_ids)):
            ...
            if args.trained_model_dir:
                config = XLNetConfig(os.path.join(args.trained_model_dir, 'config.json'))
                model = XLNetForSequenceClassification(config, num_labels=num_labels)
                ...
                print('init model...')
                # bert_config = XLNetConfig.from_json_file(os.path.join(args.bert_model, 'config.json'))
                # model = XLNetForSequenceClassification(bert_config, num_labels=num_labels)
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                eval_precision = precision_score(y_true=all_label_ids[valid_index], y_pred=results, average='macro',
                                                 zero_division='warn')
                eval_recall = recall_score(y_true=all_label_ids[valid_index], y_pred=results, zero_division='warn', average='macro')
