...
def main():
    ...
    if args.do_train:
        ...
        for fold, (train_index, valid_index) in enumerate(skf.split(all_label_ids, all_label_ids)):
            ...
            test_loader = DataLoader(test, batch_size=args.train_batch_size, shuffle=False)

            num_train_optimization_steps = int(
                len(train_label) / args.train_batch_size / args.gradient_accumulation_steps) * (
            ...
            for _ in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                eval_precision = precision_score(y_true=all_label_ids[valid_index], y_pred=results, average='binary',
                                                 zero_division='warn')
                eval_recall = recall_score(y_true=all_label_ids[valid_index], y_pred=results, zero_division='warn')
                eval_f1 = f1_score(y_true=all_label_ids[valid_index], y_pred=results)
                ...
                if best_f1 < eval_f1:
                    ...
                    torch.save(model_to_save, "./trained_models/ccks2022/pytorch_model_{}.bin".format(fold))
